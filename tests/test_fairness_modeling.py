...
def test_strategy_specs_include_expected_region_interventions():
    ...
    assert names == ["region_balanced_mild", "region3_default_boost"]
...
def test_region3_default_boost_targets_only_region3_defaults_and_normalize():
    from src.models.fairness_aware_training import _region3_default_boost_weights
    ...
    weights, details = _region3_default_boost_weights(train_groups, y_train)

    np.testing.assert_allclose(weights.mean(), 1.0)
    assert details["boosted_rows"] == 1
