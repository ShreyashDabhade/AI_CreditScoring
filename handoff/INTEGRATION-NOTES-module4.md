## Module 4 — Explainability & Fairness: Integration Notes
...
1. `model_fairness_audit_passed.joblib` is overwritten by this module. Module 5 must
   load it AFTER Module 4 has run. Load order in integration: M3 → M4 → M5.
2. The fairness audit DataFrame passed to `run_full_fairness_audit()` must contain
   ...
   split DataFrame — NOT from `build_full()` output (which drops fairness columns).
3. With only 200 rows in synthetic mode, most fairness cells will be ineligible
   (n < 200). This is expected — the audit returns `False`. Real data (~61k test rows)
   will have sufficient cell sizes.
4. `top_5_explanations_from_shap` expects a `pd.Series` indexed by feature name — not a
